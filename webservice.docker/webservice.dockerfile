...
# * cd webservice.docker/webservice/
# * Put your data into the direcotory webservice.docker/webservice/data
#
# 1)
# git rev-parse HEAD > webservice/git-commit-sha
#
# 2) Create image (takes some time), gitlab_token must be created personally within gitlab rep (Project Access Token)
# cd into webservice.docker
# sudo docker build --tag s6et_webservice -f webservice.dockerfile . --platform linux/amd64 --build-arg gitlab_token=XXX
#
...
# Webservice
ADD servicehub /webservice/
#WORKDIR /tmp

#RUN git clone https://docker_webservice:$gitlab_token@mygit.th-deg.de/tcf-geo/webservice.git
